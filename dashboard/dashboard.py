...
def create_byseason_df(df):
    byseason_df = df.groupby(by="season").cnt_x.sum().reset_index()
...
#create_byregistered_df()
def create_byregistered_df(df):
    byregistered_df = df.groupby(by="dteday").registered_x.sum().reset_index()
...
def create_bycasual_df(df):
    bycasual_df = df.groupby(by="dteday").casual_x.sum().reset_index()
...
st.subheader("Number of Users Based on..")
...
ax.set_title("Users by Season", loc="center", fontsize=50)
